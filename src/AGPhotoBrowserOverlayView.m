...
- (void)layoutSubviews
{
	...
	if (self.descriptionExpanded) {
        ...
        CGSize maximumLabelSize = CGSizeMake(CGRectGetWidth([UIScreen mainScreen].bounds) - 40, MAXFLOAT);

        NSDictionary *stringAttributes = [NSDictionary dictionaryWithObject:self.descriptionLabel.font forKey: NSFontAttributeName];
        
        CGSize descriptionSize = [self.description boundingRectWithSize:maximumLabelSize
	...
	if ([self.descriptionLabel.text length]) {
        ...
        CGSize maximumLabelSize = CGSizeMake(self.descriptionLabel.frame.size.width, MAXFLOAT);
        
        NSDictionary *stringAttributes = [NSDictionary dictionaryWithObject:self.descriptionLabel.font forKey: NSFontAttributeName];
        
        CGSize descriptionSize = [self.descriptionLabel.text boundingRectWithSize:maximumLabelSize
                                                            options:NSStringDrawingTruncatesLastVisibleLine|NSStringDrawingUsesLineFragmentOrigin
                                                         attributes:stringAttributes context:nil].size;
