...
#define POPLISTVIEW_SCREENINSET 50.//44
...
- (id)initWithTitle:(NSString *)aTitle options:(NSArray *)aOptions {
    ...
    if (self = [super initWithFrame:rect]) {
        ...
        if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone){
            _tableView = [[UITableView alloc] initWithFrame:CGRectMake(80 -POPLISTVIEW_SCREENINSET,
                                                                   POPLISTVIEW_SCREENINSET + POPLISTVIEW_HEADER_HEIGHT,
                                                                   rect.size.width - 2 * POPLISTVIEW_SCREENINSET + 40,
                                                                   rect.size.height - 2 * POPLISTVIEW_SCREENINSET - POPLISTVIEW_HEADER_HEIGHT - RADIUS)];
        } else {
            _tableView = [[UITableView alloc] initWithFrame:CGRectMake(180 -POPLISTVIEW_SCREENINSET,
                                                                       POPLISTVIEW_SCREENINSET + POPLISTVIEW_HEADER_HEIGHT+ 100,
                                                                       rect.size.width - 2 * POPLISTVIEW_SCREENINSET - 180,
                                                                       rect.size.height - 2 * POPLISTVIEW_SCREENINSET - POPLISTVIEW_HEADER_HEIGHT - RADIUS- 200)];
        }
...
- (void)drawRect:(CGRect)rect {
     ...
     if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone){
         ...
         [[UIColor colorWithWhite:0 alpha:1] setFill];//.75
         
         
         float x = POPLISTVIEW_SCREENINSET -40;
       ...
       //      [_title drawInRect:titleRect withFont:[UIFont systemFontOfSize:16.]];// Pre iOS 7
               ...
               [_title drawInRect:titleRect withAttributes:@{NSForegroundColorAttributeName: [UIColor colorWithRed:0.020 green:0.549 blue:0.961 alpha:1.],NSFontAttributeName:[UIFont systemFontOfSize:16.] }];
         ...
         float x = POPLISTVIEW_SCREENINSET +80;
         float y = POPLISTVIEW_SCREENINSET +100;
         float width = bgRect.size.width ;
         float height = bgRect.size.height;
         CGMutablePathRef path = CGPathCreateMutable();
         CGPathMoveToPoint(path, NULL, x, y + RADIUS);
