...
static NSInteger const kNotificationViewEmptySymbolViewTag = 666;
                    ...
                    duration:(NSTimeInterval)duration
{
    ...
    __block NotificationView* note = [[NotificationView alloc] initWithParentViewController:viewController];
                     ...
                     message:(NSString *)message
{
    ...
    [NotificationView showInViewController:viewController
                                   tintColor:[NotificationView blurTintColorForStyle:style]
                                       image:[NotificationView imageForStyle:style]
                                     message:message
                                    duration:kNotificationViewDefaultShowDuration];
}
                                                        ...
                                                        message:(NSString*)message
{
    ...
    NotificationView* note = [[NotificationView alloc] initWithParentViewController:viewController];
    note.tintColor = tintColor;
    note.image = image;
    note.textLabel.text = message;
...
- (void)updateConstraints
{
    ...
    CGFloat symbolViewWidth = self.symbolView.tag != kNotificationViewEmptySymbolViewTag ?
    kNotificationViewSymbolViewSidelength : 0.0f;
    CGFloat symbolViewHeight = kNotificationViewSymbolViewSidelength;
...
- (void)dismissWithStyle:(NotificationViewStyle)style message:(NSString *)message duration:(NSTimeInterval)duration animated:(BOOL)animated
{
    ...
    [UIView animateWithDuration:0.1 animations:^{
        ...
        weakself.image = [NotificationView imageForStyle:style];
        weakself.textLabel.text = message;
        weakself.tintColor = [NotificationView blurTintColorForStyle:style];
...
- (CGRect)visibleFrame
{
    ...
    CGRect displayFrame = CGRectMake(0, 0, CGRectGetWidth(viewController.view.frame),
                                     kNotificationViewHeight + viewController.topLayoutGuide.length);
...
- (CGRect)hiddenFrame
{
    ...
    CGRect offscreenFrame = CGRectMake(0, -kNotificationViewHeight - viewController.topLayoutGuide.length,
...
- (void)updateSymbolView
{
    ...
    if (self.isShowingActivity) {
        ...
        _symbolView.tag = kNotificationViewEmptySymbolViewTag;
    }
...
+ (UIImage*)imageForStyle:(NotificationViewStyle)style
{
    UIImage* matchedImage = nil;
    switch (style) {
        case NotificationViewStyleSuccess:
            matchedImage = [UIImage imageNamed:@"CSNotificationView_checkmarkIcon"];
            break;
        case NotificationViewStyleError:
...
+ (UIColor*)blurTintColorForStyle:(NotificationViewStyle)style
{
